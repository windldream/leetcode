...
var repeatLimitedString = function (s, repeatLimit) {
  ...
  const fq = new FastPriorityQueue((a, b) => a[0].charCodeAt() > b[0].charCodeAt())
  const maxFq = new FastPriorityQueue((a, b) => a[0].charCodeAt() > b[0].charCodeAt())
  let ans = ''

  for (const [key, cnt] of map) fq.add([key, cnt])

  while (!fq.isEmpty()) {
    const [key, cnt] = fq.poll()

    if (maxFq.isEmpty()) {
      if (cnt <= repeatLimit) {
        ans += key.repeat(cnt)
      } else {
        ans += key.repeat(repeatLimit)
        maxFq.add([key, cnt - repeatLimit])
      }
    } else {
      const [lastKey, lastCnt] = maxFq.poll()

      if (lastCnt <= repeatLimit) {
        ans += key + lastKey.repeat(lastCnt)
      } else {
        ans += key + lastKey.repeat(repeatLimit)
        maxFq.add([lastKey, lastCnt - repeatLimit])
      }
