...
var SORTracker = function () {
  this.minFq = new FastPriorityQueue((a, b) => {
    if (a[1] === b[1]) {
      return a[0] > b[0]
    }
    return a[1] < b[1]
  ...
  this.maxFq = new FastPriorityQueue((a, b) => {
    if (a[1] === b[1]) {
      return a[0] < b[0]
    }
    return a[1] > b[1]
...
SORTracker.prototype.add = function (name, score) {
  this.maxFq.add([name, score])
  this.minFq.add(this.maxFq.poll())
}
...
SORTracker.prototype.get = function () {
  while (this.minFq.size < this.cur) {
    this.minFq.add(this.maxFq.poll())
  }

  while (this.minFq.size > this.cur) {
    this.maxFq.add(this.minFq.poll())
  }

  this.cur += 1

  return this.minFq.peek()[0]
}
