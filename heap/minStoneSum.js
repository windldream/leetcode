...
var minStoneSum = function (piles, k) {
  const maxQueue = new FastPriorityQueue((a, b) => a > b)
  piles.forEach((pile) => maxQueue.add(pile))

  while (k--) {
    const max = maxQueue.poll()
    maxQueue.add(max - (max >> 1))
  }

  let sum = 0
  while (!maxQueue.isEmpty()) sum += maxQueue.poll()
