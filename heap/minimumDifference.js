...
var minimumDifference = function (nums) {
  const minFq = new FastPriorityQueue()
  const m = nums.length
  ...
  const suffixSum = Array(m - n + 1).fill(0)
  suffixSum[m - n] = sum

  for (let i = m - n - 1; i >= n; i--) {
    minFq.add(nums[i])
    sum += nums[i] - minFq.poll()
    suffixSum[i] = sum
  }

  const maxFq = new FastPriorityQueue((a, b) => a > b)
  let preSum = 0
  ...
  for (let i = n; i < m - n; i++) {
    maxFq.add(nums[i])
    preSum += nums[i] - maxFq.poll()
    ans = Math.min(ans, preSum - suffixSum[i])
  }
