...
var averageHeightOfBuildings = function (buildings) {
  ...
  buildings.forEach((building) => {
    fq.add([building[0], 1, building[2]])
    fq.add([building[1], -1, -building[2]])
  })
  ...
  while (!fq.isEmpty()) {
    ...
    while (!fq.isEmpty() && fq.peek()[0] === tmp[0]) {
      const top = fq.poll()
      h += top[2]
      c += top[1]
    }
    ...
    if (cnt > 0 && c > 0) {
      if (Math.trunc(height / cnt) !== Math.trunc(h / c)) {
        ans.push([left, tmp[0], Math.trunc(height / cnt)])
        left = tmp[0]
      }
